...
# ==========================================================
# LABORATÓRIO AULA 08 - PERFIS E QUALIDADE
# ==========================================================
...
from app.services.laboratorio_service import (
    service_assumir_chamado,
    service_alterar_status,
    service_criar_comentario,
    service_listar_comentarios,
    service_listar_historico,
    service_obter_resumo_dashboard,
)


@router.post("/chamados/{chamado_id}/assumir", tags=["Laboratório Aula 08"])
def post_assumir_chamado(chamado_id: int, dados: AssumirChamado):
    resultado = service_assumir_chamado(chamado_id, dados.tecnico_id)
...
@router.patch("/chamados/{chamado_id}/status", tags=["Laboratório Aula 08"])
def patch_status_chamado(chamado_id: int, dados: AlterarStatusChamado):
    resultado = service_alterar_status(chamado_id, dados)
    if resultado is None:
        raise HTTPException(status_code=404, detail="Chamado não encontrado.")
    return {
...
@router.post("/chamados/{chamado_id}/comentarios", status_code=201, tags=["Laboratório Aula 08"])
def post_comentario(chamado_id: int, dados: ComentarioCriar):
    comentario_id = service_criar_comentario(chamado_id, dados)
    if comentario_id is None:
        raise HTTPException(status_code=404, detail="Chamado não encontrado.")
    return {"mensagem": "Comentário registrado com sucesso.", "id": comentario_id}


@router.get("/chamados/{chamado_id}/comentarios", tags=["Laboratório Aula 08"])
def get_comentarios(chamado_id: int):
    return service_listar_comentarios(chamado_id)


@router.get("/chamados/{chamado_id}/historico", tags=["Laboratório Aula 08"])
def get_historico(chamado_id: int):
    return service_listar_historico(chamado_id)


@router.get("/dashboard/resumo", tags=["Laboratório Aula 08"])
def get_dashboard_resumo():
    return service_obter_resumo_dashboard()
