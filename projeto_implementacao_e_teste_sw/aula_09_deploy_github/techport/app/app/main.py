        ...
        with tab_chamados:
            ...
            if chamados:
                ...
                chamado_id = st.selectbox(
                    ...
                    key="admin_inspecionar_chamado",
                )

                # Sempre busca o chamado selecionado diretamente na API.
                # Assim o painel de inspeção não depende de histórico/comentários
                # existirem para exibir informações.
                chamado = api_segura("GET", f"/chamados/{chamado_id}")

                if chamado:
                    st.markdown("### 🔎 Detalhes do chamado")

                    d1, d2, d3, d4 = st.columns(4)
                    d1.metric("ID", f"#{chamado.get('id')}")
                    d2.metric("Status", status_legivel(chamado.get("status")))
                    d3.metric("Prioridade", chamado.get("prioridade") or "—")
                    d4.metric("Categoria", chamado.get("categoria") or "—")

                    st.markdown(f"#### {chamado.get('titulo') or 'Sem título'}")
                    st.write(chamado.get("descricao") or "Sem descrição cadastrada.")

                    info1, info2, info3 = st.columns(3)
                    info1.write(f"**Usuário ID:** {chamado.get('usuario_id') or '—'}")
                    info2.write(f"**Técnico ID:** {chamado.get('tecnico_id') or 'Não atribuído'}")
                    info3.write(f"**Canal:** {chamado.get('canal_abertura') or '—'}")

                    datas1, datas2, datas3 = st.columns(3)
                    datas1.write(f"**Abertura:** {chamado.get('data_abertura') or '—'}")
                    datas2.write(f"**Atualização:** {chamado.get('data_atualizacao') or '—'}")
                    datas3.write(f"**Fechamento:** {chamado.get('data_fechamento') or '—'}")
                    ...
                    with col_h:
                        ...
                        if hist:
                            ...
                            st.info(
                                "Este chamado ainda não possui histórico registrado. "
                                "Chamados antigos da volumetria podem não ter eventos. "
                                "Assuma ou altere o status pelo perfil Técnico para gerar histórico."
                            )
                    ...
                    with col_c:
                        ...
                        if comentarios:
                            ...
                            st.info(
                                ...
                                "Adicione um comentário pelo perfil Usuário ou Técnico para visualizar aqui."
                            )
                else:
                    st.warning("Não foi possível carregar os detalhes do chamado selecionado.")
            else:
                st.info("Nenhum chamado cadastrado para inspeção.")
